...
class EquiVolumePlotter:
    ...
    def __init__(
        ...
        self.volume_min_excluded, self.volume_max_excluded = get_min_max_of_volume(
            self.stock_data
        )
    ...
    def get_data(self, data_src):
        if data_src == "Yahoo":
            ...
            stock_data = get_tdx_daily_data(
                ticker=self.ticker,
                start=self.start_date,
                end=self.end_date,
                interval=self.time_interval,
                tdx_dir=self.tdx_dir,
            )
            return stock_data

    def impute_data(self):
        self.stock_data = add_col_coefficient(
            self.stock_data, self.volume_min_excluded, self.volume_max_excluded
        )
        self.stock_data = add_col_bar_color(
            self.stock_data,
            self.color,
            self.volume_min_excluded,
